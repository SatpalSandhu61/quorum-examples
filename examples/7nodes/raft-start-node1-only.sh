...
function usage() {
  echo ""
  echo "Usage:"
  echo "    $0 [constellation | tessera] [--tesseraOptions \"options for Tessera start script\"]"
  echo ""
  echo "Where:"
  echo "    constellation | tessera - specifies which implementation to start up (default = tessera)"
...
constellationImpl=tessera
tesseraOptions=
while (( "$#" )); do
  case "$1" in
    constellation)
      constellationImpl=constellation
      shift
      ;;
    tessera)
      constellationImpl=tessera
...
mkdir -p qdata/logs
if [ "$constellationImpl" == "constellation" ]; then
  echo "[*] Starting Constellation nodes"
  ### ./constellation-start.sh
elif [ "$constellationImpl" == "tessera" ]; then
  echo "[*] Starting Tessera nodes"
  ### ./tessera-start.sh ${tesseraOptions}
else
  echo "Unsupported implementation: ${constellationImpl}"
  usage
fi
