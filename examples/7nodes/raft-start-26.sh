...
if [ "$privacyImpl" == "tessera" ]; then
  ...
  ./tessera-start-26.sh ${tesseraOptions}
...
for i in {1..26}; do
  raftport=`expr 50400 + ${i}`
  rpcport=`expr 21999 + ${i}`
  port=`expr 20999 + ${i}`
  PRIVATE_CONFIG=qdata/c${i}/tm.ipc nohup geth --datadir qdata/dd${i} $ARGS --permissioned --raftport ${raftport} --rpcport ${rpcport} --port ${port} --unlock 0 --password passwords.txt 2>>qdata/logs/${i}.log &
done
set +v
